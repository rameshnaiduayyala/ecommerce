...
const ProfilePage = () => {
  ...
  useEffect(() => {
    ...
    const loadProfileData = async () => {
      // 1. First set from auth user metadata
      ...
      const initialData = {
        ...
        city: meta.city || '',
        postalCode: meta.postalCode || meta.postal_code || '',
        country: meta.country || 'India'
      };

      // 2. Try fetching from public.users table for full_name
      try {
        ...
        if (data && !error) {
          initialData.fullName = data.full_name || initialData.fullName;
        }
      } catch (err) {
        console.warn("Could not load full name from users table:", err);
      }
  ...
  const handleUpdateProfile = async (e) => {
    ...
    try {
      ...
      // 3. Refresh user state in context to reflect metadata changes in UI instantly
      await refreshUser();
