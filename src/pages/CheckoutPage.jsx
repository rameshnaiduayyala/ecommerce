  ...
  const handleCheckout = async (e) => {
    ...
    try {
      const { data: orderData, error: orderError } = await supabase
        .from('orders').insert([{ user_id: user.id, total_amount: grandTotal, status: 'pending', shipping_address: formData }]).select().single();
      if (orderError) throw orderError;

      const { error: itemsError } = await supabase.from('order_items').insert(
        cartItems.map(item => ({ order_id: orderData.id, product_id: item.id, quantity: item.quantity, price_at_time: item.discount_price || item.price }))
      );
